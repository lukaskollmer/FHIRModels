...
private func parse(
    ...
    switch shape {
    case .date:
        return .date(try parser.dateComponents(from: input))
    case .time:
        return .time(try parser.timeComponents(from: input))
    case .dateTime:
...
//                let spelling = (parsed.time?.timeZoneString).map { #""\#($0)""# } ?? "nil"
//                return .success("dateTime(\(render(parsed.date)), \(time), \(seconds), \(spelling))")
        return .dateTime(try parser.dateTimeComponents(from: input))
    case .instant:
//                let parsed = try engine.instantComponents(from: descriptor.input)
//                return .success(#"instant(\#(render(parsed.date)), \#(render(parsed.time.time)), \#(parsed.time.timeZone.secondsFromGMT()), "\#(parsed.time.timeZoneString)")"#)
        return .instant(try parser.instantComponents(from: input))
    case .instantDate:
//                return try .success(parser.instantDateComponents(from: descriptor.input))
        return .instantDate(try parser.instantDateComponents(from: input))
    case .timeZone:
//                let parsed = try engine.timeZoneComponents(from: descriptor.input)
//                return .success(#"tz(\#(parsed.secondsFromGMT), "\#(parsed.timeZoneString)")"#)
        return .timeZone(try parser.timeZoneComponents(from: input))
    }
}
...
//        let _: Instant = "2017-01-01T00:00:47Z"
        ...
        _ = try NewDateTimeParser.dateComponents(from: "2027-07-17")
        _ = try NewDateTimeParser.dateComponents(from: "0001-01-01")
        #expect(throws: (any Error).self) {
            _ = try NewDateTimeParser.dateComponents(from: "0001-00-00")
        }
        #expect(throws: (any Error).self) {
            _ = try NewDateTimeParser.dateComponents(from: "0000-00-00")
        }
        #expect(throws: (any Error).self) {
            _ = try NewDateTimeParser.dateComponents(from: "0000-00")
        }
        #expect(throws: (any Error).self) {
            _ = try NewDateTimeParser.dateComponents(from: "0000")
        }
...
extension DateTimeParserEngineAgreementTests {
	...
	fileprivate func longFractionAgreement(engine: any DateTimeParserProtocol.Type) throws {
		...
		let parsed = try engine.timeComponents(from: string)
	...
	private static let corpus: [DivergenceDescriptor] = [
		...
		.init(.dateTime, "2015-02-07T13:28:17+-05:00", scanner: .success(.dateTime(ParsedDateTime(
			...
			time: ParsedDateTime.TimeComponent(
				time: ParsedTime(hour: 13, minute: 28, second: Decimal(string: "17")!, originalSecondsString: "17"),
		...
		.init(.instant, "2017-12-09t09:30:51Z", scanner: .success(.instant(ParsedInstant(
			...
			time: ParsedDateTime.TimeComponent(
				time: ParsedTime(hour: 9, minute: 30, second: Decimal(string: "51")!, originalSecondsString: "51"),
				timeZone: TimeZone(secondsFromGMT: 0)!,
				timeZoneString: "Z"
			)
		...
		.init(.time, "13:28:17.٢٣٩", scanner: .success(.time(ParsedTime(hour: 13, minute: 28, second: Decimal(string: "17.٢٣٩")!, originalSecondsString: "17.٢٣٩"))), newParser: .failure("invalidSecond@9")),
...
//		)))),
		...
		.init(.time, "13:28:17.2٣9", scanner: .success(.time(ParsedTime(hour: 13, minute: 28, second: Decimal(string: "17.2٣9")!, originalSecondsString: "17.2٣9"))), newParser: .failure("additionalCharacters@10")),
	...
	/// Documents which engine is active. When this fails because the typealias was flipped to the
	/// new engine: update this expectation, promote the ScannerParsingKnownIssue tests into plain
	/// characterization assertions, and revisit the public-API pins that encode scanner bugs
	/// (e.g. FHIRTimeTests' rejection of "09:41:60.0001").
	@Test
	func activeEngineIsScanner() {
		#expect(DateTimeParser.self == ScannerDateTimeParser.self)
	}
}
