...
public indirect enum RequestIntent: String, FHIRPrimitiveType {
	...
	/// The request is a response to a solicitation from another party for an offer for a service (e.g. a provider
	/// offering their services or providing a quote for a service).
	case offerResponse = "offer-response"
	
	/// The request represents a component or option for a RequestOrchestration that establishes timing, conditionality
	/// and/or other constraints among a set of requests.  Refer to [[[RequestOrchestration]]] for additional
	/// information on how this status is used.
	case option
	...
	/// The request is a solicitation from one party for an offer from another party for a service (e.g. a request for a
	/// provider to offer their services or a request to quote a price).
	case solicitOffer = "solicit-offer"
}
